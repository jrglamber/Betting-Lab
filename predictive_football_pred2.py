...
APP_VERSION = "0.16.1"
EXPERIMENT_VERSION = "PRED2_DIXON_COLES"
MARKET_EXPERIMENT_VERSION = "PRED2_DIXON_COLES_MARKETS_V1"
MODEL_VERSION = "PRED2_DIXON_COLES_V1"
...
def _config_hash(settings, execution_books: Sequence[str]) -> str:
    payload = {
        ...
        "pred2_enabled": bool(getattr(settings, "predictive_football_pred2_enabled", True)),
        "pred2_rho_min": float(getattr(settings, "predictive_football_pred2_rho_min", -0.20)),
        "pred2_rho_max": float(getattr(settings, "predictive_football_pred2_rho_max", 0.20)),
        "pred2_rho_step": float(getattr(settings, "predictive_football_pred2_rho_step", 0.01)),
    }
...
def _dc_tau(home_goals: int, away_goals: int, home_lambda: float, away_lambda: float, rho: float) -> float:
    # Dixon-Coles low-score dependency correction. rho=0 exactly recovers
    # the independent-Poisson PRED1 benchmark.
...
def _dc_score_matrix(home_lambda: float, away_lambda: float, rho: float, max_goals: int = 10) -> List[Tuple[int,int,float]]:
    raw: List[Tuple[int,int,float]] = []
    total = 0.0
    for i in range(max_goals + 1):
        ph = _poisson_pmf(i, home_lambda)
        for j in range(max_goals + 1):
            p = ph * _poisson_pmf(j, away_lambda) * _dc_tau(i,j,home_lambda,away_lambda,rho)
            p = max(0.0, p)
            raw.append((i,j,p))
            total += p
    if total <= 0:
        return [(i,j,p) for i,j,p in _dc_score_matrix(home_lambda,away_lambda,0.0,max_goals)] if abs(rho)>1e-12 else []
    return [(i,j,p/total) for i,j,p in raw]


def score_probabilities(home_lambda: float, away_lambda: float, max_goals: int = 10, rho: float = 0.0) -> Tuple[float, float, float]:
    home = draw = away = 0.0
    for i,j,p in _dc_score_matrix(home_lambda,away_lambda,rho,max_goals):
        if i > j:
            home += p
        elif i == j:
            draw += p
...
def btts_probabilities(home_lambda: float, away_lambda: float, rho: float = 0.0) -> Tuple[float, float]:
    yes=sum(p for i,j,p in _dc_score_matrix(home_lambda,away_lambda,rho) if i>=1 and j>=1)
    yes=max(0.0,min(1.0,yes))
    return yes,1.0-yes


def total_probabilities(home_lambda: float, away_lambda: float, point: float, rho: float = 0.0) -> Tuple[float, float]:
    under=sum(p for i,j,p in _dc_score_matrix(home_lambda,away_lambda,rho) if (i+j)<float(point))
...
def derived_market_probabilities(
    ...
    if "btts" in enabled:
        yes,no=btts_probabilities(home_lambda,away_lambda,rho)
        out.extend([
    ...
    if "totals" in enabled:
        for raw_point in total_points:
            ...
            if abs((point*2.0)-round(point*2.0))>1e-9 or abs(point-round(point))<1e-9:
                continue
            over,under=total_probabilities(home_lambda,away_lambda,point,rho)
            line=f"{point:.1f}"
            out.extend([
...
class PredictiveFootballPred2Engine:
    def __init__(
        self,
        db: Database,
        settings,
    ...
    def enabled(self) -> bool:
        return bool(getattr(self.settings, "predictive_football_pred2_enabled", True))
    ...
    def fit_event(
        ...
        *,
        ...
        # Cache PRED1-style attack/defence strengths, then fit only the
        # Dixon-Coles dependency parameter on the same pre-kickoff history.
        # This deliberately changes score-distribution mechanics without
        # changing the underlying information set.
        strength_cache: Dict[str, Tuple[float,float,float]] = {}
        for name in names:
            strength_cache[_normalize_text(name)] = team_strength(name)
        h_att,h_def,h_eff = strength_cache[_normalize_text(mapped_home)]
        a_att,a_def,a_eff = strength_cache[_normalize_text(mapped_away)]
        ...
        rho_min=float(getattr(self.settings,"predictive_football_pred2_rho_min",-0.20))
        rho_max=float(getattr(self.settings,"predictive_football_pred2_rho_max",0.20))
        rho_step=max(0.0025,float(getattr(self.settings,"predictive_football_pred2_rho_step",0.01)))
        rho_min,rho_max=min(rho_min,rho_max),max(rho_min,rho_max)
        candidates=[]
        value=rho_min
        while value <= rho_max + 1e-12:
            candidates.append(round(value,6)); value += rho_step
        if 0.0 not in candidates:
            candidates.append(0.0)
        best_rho=0.0; best_ll=float("-inf")
        low_cells=((0,0),(0,1),(1,0),(1,1))
        for rho in candidates:
            # A Dixon-Coles rho is admissible only if every corrected low-score
            # probability stays positive for the target event and the fitted
            # historical score-intensity pairs. Never clip an invalid tau.
            if any(_dc_tau(i,j,home_lambda,away_lambda,rho)<=1e-9 for i,j in low_cells):
                continue
            ll=0.0; valid=True; informative=0
            for r,w in weighted:
                hg=int(r["home_goals"]); ag=int(r["away_goals"])
                if hg>1 or ag>1:
                    continue
                hs=strength_cache.get(_normalize_text(str(r["home_team"])))
                aas=strength_cache.get(_normalize_text(str(r["away_team"])))
                if not hs or not aas:
                    continue
                rh_att,rh_def,_=hs; ra_att,ra_def,_=aas
                lh=max(0.20,min(4.50,league_home_mean*rh_att*ra_def))
                la=max(0.20,min(4.50,league_away_mean*ra_att*rh_def))
                if any(_dc_tau(i,j,lh,la,rho)<=1e-9 for i,j in low_cells):
                    valid=False; break
                tau=_dc_tau(hg,ag,lh,la,rho)
                ll += float(w)*math.log(tau)
                informative += 1
            if valid and informative and ll>best_ll:
                best_ll=ll; best_rho=float(rho)

        hp,dp,ap = score_probabilities(home_lambda,away_lambda,rho=best_rho)
    ...
    def freeze_due_predictions(self, now: Optional[datetime] = None) -> Dict[str, int]:
        ...
        for event in events:
            existing = self.db.fetchone(
                "SELECT id FROM football_predictive2_predictions WHERE event_id=?",
                (event["event_id"],),
            )
            if existing:
                continue
            ...
            self.db.execute(
                ...
                INSERT INTO football_predictive2_predictions(
                    ...
                    expected_home_goals,expected_away_goals,dixon_coles_rho,
                ...
                (
                    ...
                    fit["expected_home_goals"],fit["expected_away_goals"],fit["dixon_coles_rho"],
    ...
    def ensure_market_predictions(self) -> int:
        ...
        for pred in predictions:
            rows = derived_market_probabilities(
                ...
                markets,
                total_points,
                float(pred.get("dixon_coles_rho") or 0.0),
            )
            for row in rows:
                exists = self.db.fetchone(
                    """
                    SELECT id FROM football_predictive2_market_predictions
                ...
                self.db.execute(
                    """
                    INSERT INTO football_predictive2_market_predictions(
    ...
    def evaluate_market_predictions(
        ...
        rows = self.db.fetchall(
            ...
            JOIN football_predictive2_predictions p ON p.id=mp.prediction_id
        ...
        for mp in rows:
            for wave in self._market_waves(mp,str(mp["market_key"])):
                exists_eval = self.db.fetchone(
                    """
                    SELECT id FROM football_predictive2_market_evaluations
                ...
                if not quotes:
                    self.db.execute(
                        """
                        INSERT INTO football_predictive2_market_evaluations(
                ...
                frozen=self.db.fetchone(
                    """
                    SELECT id FROM football_predictive2_market_bets
                ...
                if frozen:
                    ...
                    self.db.execute(
                        """
                        INSERT INTO football_predictive2_market_bets(
    ...
    def track_market_prices(self, now: Optional[datetime]=None) -> int:
        ...
        bets=self.db.fetchall(
            ...
            JOIN football_predictive2_predictions p ON p.id=b.prediction_id
            ORDER BY b.id
            """
        )
        inserted=0
        for bet in bets:
            ...
            for w in waves:
                ...
                exists=self.db.fetchone(
                    """
                    SELECT id FROM football_predictive2_market_price_observations
                ...
                self.db.execute(
                    """
                    INSERT INTO football_predictive2_market_price_observations(
    ...
    def finalize_market_clv(self, now: Optional[datetime]=None) -> int:
        ...
        for bet in bets:
            ...
            self.db.execute(
                """
                UPDATE football_predictive2_market_bets
    ...
    def settle_market_predictions(self) -> Dict[str,int]:
        ...
        for mp in market_predictions:
            ...
            self.db.execute(
                """
                UPDATE football_predictive2_market_predictions
            ...
            for wave in self._unprocessed_waves(pred):
                ...
                for selection,(prob,fair) in probs.items():
                    existing_eval = self.db.fetchone(
                        """
                        SELECT id FROM football_predictive2_evaluations
                    ...
                    if not quotes:
                        self.db.execute(
                            """
                            INSERT INTO football_predictive2_evaluations(
                    ...
                    frozen = self.db.fetchone(
                        "SELECT id FROM football_predictive2_bets WHERE predictive_key=?",
                        (key,),
                    )
                    if frozen:
    ...
    def track_prices(self, now: Optional[datetime] = None) -> int:
        ...
        for bet in bets:
            ...
            for w in waves:
                ...
                exists=self.db.fetchone(
                    """
                    SELECT id FROM football_predictive2_price_observations
                    WHERE predictive_bet_id=? AND source_snapshot_at=?
                    """,
                    (bet["id"],wave),
                )
                ...
                self.db.execute(
                    """
                    INSERT INTO football_predictive2_price_observations(
                        predictive_bet_id,observed_at,source_snapshot_at,
                        bookmaker_key,price,move_vs_entry_pct
                    ) VALUES(?,?,?,?,?,?)
                    """,
    ...
    def finalize_clv(self, now: Optional[datetime] = None) -> int:
        ...
        for bet in bets:
            ...
            self.db.execute(
                """
                UPDATE football_predictive2_bets
                SET closing_odds=?,clv_pct=?,closing_observed_at=?,
                    closing_minutes_before_kickoff=?,clv_quality=?
                WHERE id=?
                """,
    ...
    def finalize_prediction_market_close(self, now: Optional[datetime] = None) -> int:
        ...
        for pred in preds:
            ...
            self.db.execute(
                """
                UPDATE football_predictive2_predictions
    ...
    def maintenance(self, now: Optional[datetime]=None) -> Dict[str,Any]:
        now=now or datetime.now(timezone.utc)
        if bool(getattr(self.settings,"predictive_football_enabled",True)):
            internal=0
            bootstrap={"enabled":False,"shared_with_pred1":True,"attempted":0,"imported":0}
        else:
            # Standalone fallback only if PRED1 is explicitly disabled. Under
            # normal paired operation PRED1 owns the shared free-data ingest.
...
def predictive2_scoreboard(db: Database) -> Dict[str,Any]:
    preds=db.fetchall(
        "SELECT * FROM football_predictive2_predictions ORDER BY id"
    )
...
def predictive2_market_summary(db: Database) -> List[Dict[str,Any]]:
...
def predictive2_market_funnel(db: Database) -> List[Dict[str,Any]]:
    return db.fetchall(
        """
        SELECT market_key,line_key,decision,reason,COUNT(*) AS count
        FROM football_predictive2_market_evaluations
...
def latest_predictive2_market_bets(
    ...
    return db.fetchall(
        ...
        SELECT b.*,p.league,p.home_team,p.away_team,p.commence_time,
               ...
               (SELECT o.move_vs_entry_pct
                FROM football_predictive2_market_price_observations o
...
def predictive2_funnel(db: Database) -> List[Dict[str,Any]]:
    return db.fetchall(
        """
        SELECT decision,reason,COUNT(*) AS count
        FROM football_predictive2_evaluations
...
def predictive2_league_summary(db: Database) -> List[Dict[str,Any]]:
    return db.fetchall(
        """
        ...
        FROM football_predictive2_predictions
...
def latest_predictive2_bets(db: Database, limit: int=100) -> List[Dict[str,Any]]:
    return db.fetchall(
        """
        SELECT b.*,p.league,p.home_team,p.away_team,p.commence_time,
               p.expected_home_goals,p.expected_away_goals,
               (SELECT o.move_vs_entry_pct
                FROM football_predictive2_price_observations o
                WHERE o.predictive_bet_id=b.id
                ORDER BY o.source_snapshot_at DESC LIMIT 1) AS latest_move_pct
        FROM football_predictive2_bets b
        JOIN football_predictive2_predictions p ON p.id=b.prediction_id
