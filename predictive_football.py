...
class PredictiveFootballEngine:
    ...
    def ensure_market_predictions(self) -> int:
        ...
        predictions = self.db.fetchall(
            "SELECT home_probability,draw_probability,away_probability,home_team,away_team,actual_outcome,brier_score,log_loss,model_brier_advantage,closing_market_quality FROM football_predictive_predictions"
        )
        created = 0
...
def predictive_scoreboard(db: Database) -> Dict[str,Any]:
    ...
    h2h_bets=db.fetchall(
        "SELECT pnl_units,net_pnl_units,strong_candidate,clv_pct,clv_quality FROM football_predictive_bets"
    )
    market_bets=db.fetchall(
        "SELECT pnl_units,net_pnl_units,strong_candidate,clv_pct,clv_quality,market_key FROM football_predictive_market_bets"
    )
    ...
    market_predictions=db.fetchall(
        "SELECT prediction_id,market_key,line_key,brier_score FROM football_predictive_market_predictions"
    )
    derived_cases={
        (
