...
APP_VERSION = "0.19.0"
EXPERIMENT_VERSION = "PRED4_PROXY_XG"
MARKET_EXPERIMENT_VERSION = "PRED4_PROXY_XG_MARKETS_V1"
MODEL_VERSION = "PRED4_PROXY_XG_POISSON_V1"
...
ALIAS_NORMALIZATION = {
    ...
    "benfica": "sl benfica",
    # Current API-Football vs Odds API naming aliases observed in PXG1/PRED4.
...
def _config_hash(settings, execution_books: Sequence[str]) -> str:
    payload = {
        ...
        "min_league_matches": int(getattr(settings,"predictive_football_pred4_min_league_matches",20)),
        "min_team_matches": float(getattr(settings,"predictive_football_pred4_min_team_matches",2.5)),
        "prior_matches": float(getattr(settings,"predictive_football_pred4_prior_matches",4.0)),
        "half_life_days": float(getattr(settings,"predictive_football_pred4_half_life_days",365.0)),
        "lookback_days": int(getattr(settings,"predictive_football_pred4_lookback_days",1200)),
        "max_data_age_days": float(getattr(settings,"predictive_football_pred4_max_data_age_days",1200)),
...
# v0.16.1 — PRED3 uses PXG1 Open Data event xG as a genuinely different
...
class PredictiveFootballPred4Engine:
    def __init__(
        self,
        db: Database,
        settings,
    ...
    def enabled(self) -> bool:
        return bool(getattr(self.settings, "predictive_football_pred4_enabled", True))

    def sync_internal_results(self) -> int:
        # PRED4 learns only from completed current PXG1 team-match histories.
        return 0

    def _bootstrap_source_due(self, source_key: str, now: datetime) -> bool:
        row = self.db.fetchone(
            "SELECT * FROM football_predictive4_source_state WHERE source_key=?",
            (source_key,),
        )
        ...
        if not row.get("last_success_at"):
            return elapsed >= 3600
        return elapsed >= max(
            3600, int(getattr(self.settings, "predictive_football_pred4_statsbomb_refresh_seconds", 86400))
        )
    ...
    def _set_source_state(
        ...
        *,
        ...
        existing = self.db.fetchone(
            "SELECT source_key FROM football_predictive4_source_state WHERE source_key=?",
            (source_key,),
        )
        if existing:
            if success:
                self.db.execute(
                    """
                    UPDATE football_predictive4_source_state
            ...
            self.db.execute(
                """
                INSERT INTO football_predictive4_source_state(
    ...
    def _statsbomb_get_json(self, path: str) -> Any:
        base = str(getattr(
            self.settings,
            "predictive_football_pred4_statsbomb_base_url",
    ...
    def _statsbomb_catalogue_due(self, now: datetime) -> bool:
        ...
        return elapsed >= max(3600,int(getattr(
            self.settings,"predictive_football_pred4_statsbomb_refresh_seconds",86400
        )))

    def sync_statsbomb_catalogue(self, now: Optional[datetime]=None) -> Dict[str,int]:
        now=now or datetime.now(timezone.utc)
        existing=int((self.db.fetchone(
            "SELECT COUNT(*) AS n FROM football_predictive4_statsbomb_manifest"
        ) or {}).get("n") or 0)
        ...
        try:
            ...
            if not isinstance(competitions,list):
                raise ValueError("PXG1 competitions.json was not a list")
            ...
            per_comp=max(1,int(getattr(
                self.settings,"predictive_football_pred4_statsbomb_seasons_per_competition",4
            )))
            selected=[]
            ...
            for comp in selected:
                ...
                for m in matches:
                    ...
                    exists=self.db.fetchone(
                        "SELECT match_id FROM football_predictive4_statsbomb_manifest WHERE match_id=?",
                        (match_id,),
                    )
                    if exists:
                        continue
                    self.db.execute(
                        """INSERT INTO football_predictive4_statsbomb_manifest(
    ...
    def _import_statsbomb_match(self, manifest: Mapping[str,Any], now: datetime) -> Tuple[bool,str]:
        ...
        self.db.execute(
            """INSERT INTO football_predictive4_training_matches(
        ...
        self.db.execute(
            "UPDATE football_predictive4_statsbomb_manifest SET status='IMPORTED',last_error=NULL,updated_at=? WHERE match_id=?",
            (now.isoformat(),match_id),
        )
        return True,"OK"

    def bootstrap_historical_data(self, now: Optional[datetime] = None) -> Dict[str, Any]:
        # PXG1 owns all provider collection and proxy-model fitting. PRED4 makes
        # zero provider calls and consumes only already-scored current matches.
        scored = int((self.db.fetchone(
            "SELECT COUNT(*) AS n FROM football_pxg_current_matches WHERE home_proxy_xg IS NOT NULL AND away_proxy_xg IS NOT NULL"
        ) or {}).get("n") or 0)
        return {"enabled": True, "provider_calls": 0, "scored_current_matches": scored}
    ...
    def _training_rows(
        ...
        *,
        ...
        cutoff = min(training_cutoff or kickoff, kickoff)
        lookback = int(getattr(self.settings, "predictive_football_pred4_lookback_days", 60))
        start = cutoff - timedelta(days=lookback)
        return self.db.fetchall(
            """SELECT id,fixture_id,sport_key,league_name AS competition_name,played_at,
                      home_team,away_team,home_goals,away_goals,
                      home_proxy_xg AS home_xg,away_proxy_xg AS away_xg
               FROM football_pxg_current_matches
               WHERE sport_key=? AND played_at<? AND played_at>=?
                 AND home_proxy_xg IS NOT NULL AND away_proxy_xg IS NOT NULL
    ...
    def fit_event(
        ...
        *,
        ...
        cutoff = min(training_cutoff or now, kickoff)
        rows = self._training_rows(str(event["sport_key"]), kickoff, training_cutoff=cutoff)
        min_league = int(getattr(self.settings, "predictive_football_pred4_min_league_matches", 20))
        if len(rows) < min_league:
            return None, "INSUFFICIENT_PXG_LEAGUE_HISTORY"
        ...
        if not hm:
            return None, "PXG_HOME_TEAM_UNMAPPED"
        if not am:
            return None, "PXG_AWAY_TEAM_UNMAPPED"
        mapped_home, home_map_score = hm
        mapped_away, away_map_score = am

        # Require a real repeated current-season history before any forecast.
        home_rows = [r for r in rows if _normalize_text(str(r["home_team"])) == _normalize_text(mapped_home) or _normalize_text(str(r["away_team"])) == _normalize_text(mapped_home)]
        away_rows = [r for r in rows if _normalize_text(str(r["home_team"])) == _normalize_text(mapped_away) or _normalize_text(str(r["away_team"])) == _normalize_text(mapped_away)]
        min_raw = int(getattr(self.settings, "predictive_football_pred4_min_team_matches", 5))
        if len(home_rows) < min_raw:
            return None, "INSUFFICIENT_PXG_HOME_HISTORY"
        if len(away_rows) < min_raw:
            return None, "INSUFFICIENT_PXG_AWAY_HISTORY"

        half_life = float(getattr(self.settings, "predictive_football_pred4_half_life_days", 21.0))
        weighted = []
        for r in rows:
            played = parse_iso(r["played_at"])
            days = max(0.0, (cutoff - played).total_seconds() / 86400.0)
            weighted.append((r, _weight(days, half_life)))
        total_w = sum(w for _, w in weighted)
        if total_w <= 0:
            return None, "NO_EFFECTIVE_PXG_HISTORY"
        ...
        league_home_mean = max(0.20, league_home_mean)
        league_away_mean = max(0.20, league_away_mean)
        base_xg = max(0.30, (league_home_mean + league_away_mean) / 2.0)
        prior = max(0.0, float(getattr(self.settings, "predictive_football_pred4_prior_matches", 3.0)))
        ...
        def team_strength(team_name: str) -> Tuple[float, float, float, Optional[datetime]]:
            ...
            for r, w in weighted:
                played = parse_iso(r["played_at"])
                is_home = _normalize_text(str(r["home_team"])) == canonical
                is_away = _normalize_text(str(r["away_team"])) == canonical
                if is_home:
                    scored += float(r["home_xg"]) * w
                    conceded += float(r["away_xg"]) * w
                    baseline_scored += league_home_mean * w
                    baseline_conceded += league_away_mean * w
                elif is_away:
                    scored += float(r["away_xg"]) * w
                    conceded += float(r["home_xg"]) * w
        ...
        min_eff = float(getattr(self.settings, "predictive_football_pred4_min_effective_team_matches", 3.0))
        if h_eff < min_eff:
            return None, "INSUFFICIENT_EFFECTIVE_PXG_HOME_HISTORY"
        if a_eff < min_eff:
            return None, "INSUFFICIENT_EFFECTIVE_PXG_AWAY_HISTORY"
        if h_latest is None or a_latest is None:
            return None, "NO_RECENT_PXG_TEAM_HISTORY"
        age = max((cutoff - h_latest).total_seconds() / 86400.0, (cutoff - a_latest).total_seconds() / 86400.0)
        max_age = float(getattr(self.settings, "predictive_football_pred4_max_data_age_days", 30.0))
        if age > max_age:
            return None, "PXG_HISTORY_TOO_STALE"
        ...
        return {
            ...
            "pxg_data_age_days": age, "home_latest_pxg_at": h_latest.isoformat(),
            "away_latest_pxg_at": a_latest.isoformat(),
        }, "OK"
    ...
    def freeze_due_predictions(self, now: Optional[datetime] = None) -> Dict[str, int]:
        ...
        for event in events:
            if self.db.fetchone("SELECT id FROM football_predictive4_predictions WHERE event_id=?",(event["event_id"],)):
                continue
            fit,reason=self.fit_event(event,now=now)
            if not fit:
                skipped+=1; reasons[reason]=reasons.get(reason,0)+1; continue
            self.db.execute(
                """INSERT INTO football_predictive4_predictions(
                    ...
                    home_effective_matches,away_effective_matches,pxg_data_age_days,
                    home_latest_pxg_at,away_latest_pxg_at,model_version,config_hash
                ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (event["event_id"],now.isoformat(),event["sport_key"],event["league"],event["commence_time"],
                 ...
                 fit["away_effective_matches"],fit["pxg_data_age_days"],fit["home_latest_pxg_at"],
                 fit["away_latest_pxg_at"],MODEL_VERSION,self.config_hash),
            )
            created+=1
        if created or skipped:
            self.db.record_collector_run(
                "PREDICTIVE_FOOTBALL_PRED4_FORECAST",True,
    ...
    def ensure_market_predictions(self) -> int:
        ...
        for pred in predictions:
            ...
            for row in rows:
                exists = self.db.fetchone(
                    """
                    SELECT id FROM football_predictive4_market_predictions
                ...
                self.db.execute(
                    """
                    INSERT INTO football_predictive4_market_predictions(
    ...
    def evaluate_market_predictions(
        ...
        rows = self.db.fetchall(
            ...
            FROM football_predictive4_market_predictions mp
            JOIN football_predictive4_predictions p ON p.id=mp.prediction_id
        ...
        for mp in rows:
            for wave in self._market_waves(mp,str(mp["market_key"])):
                exists_eval = self.db.fetchone(
                    """
                    SELECT id FROM football_predictive4_market_evaluations
                ...
                if not quotes:
                    self.db.execute(
                        """
                        INSERT INTO football_predictive4_market_evaluations(
                ...
                frozen=self.db.fetchone(
                    """
                    SELECT id FROM football_predictive4_market_bets
                ...
                if frozen:
                    ...
                    self.db.execute(
                        """
                        INSERT INTO football_predictive4_market_bets(
    ...
    def track_market_prices(self, now: Optional[datetime]=None) -> int:
        ...
        bets=self.db.fetchall(
            ...
            FROM football_predictive4_market_bets b
            JOIN football_predictive4_predictions p ON p.id=b.prediction_id
            ORDER BY b.id
            """
        )
        inserted=0
        for bet in bets:
            ...
            for w in waves:
                ...
                exists=self.db.fetchone(
                    """
                    SELECT id FROM football_predictive4_market_price_observations
                ...
                self.db.execute(
                    """
                    INSERT INTO football_predictive4_market_price_observations(
    ...
    def finalize_market_clv(self, now: Optional[datetime]=None) -> int:
        ...
        for bet in bets:
            ...
            self.db.execute(
                """
                UPDATE football_predictive4_market_bets
    ...
    def settle_market_predictions(self) -> Dict[str,int]:
        ...
        for mp in market_predictions:
            ...
            self.db.execute(
                """
                UPDATE football_predictive4_market_predictions
    ...
    def evaluate_predictions(self) -> int:
        predictions = self.db.fetchall(
            """
            SELECT * FROM football_predictive4_predictions
            WHERE commence_time>?
            ...
            for wave in self._unprocessed_waves(pred):
                ...
                for selection,(prob,fair) in probs.items():
                    existing_eval = self.db.fetchone(
                        """
                        SELECT id FROM football_predictive4_evaluations
                    ...
                    if not quotes:
                        self.db.execute(
                            """
                            INSERT INTO football_predictive4_evaluations(
                    ...
                    frozen = self.db.fetchone(
                        "SELECT id FROM football_predictive4_bets WHERE predictive_key=?",
                        (key,),
                    )
                    if frozen:
                        ...
                        self.db.execute(
                            """
                            INSERT INTO football_predictive4_bets(
    ...
    def track_prices(self, now: Optional[datetime] = None) -> int:
        ...
        bets = self.db.fetchall(
            ...
            FROM football_predictive4_bets b
            JOIN football_predictive4_predictions p ON p.id=b.prediction_id
            ORDER BY b.id
            """
        )
        ...
        for bet in bets:
            ...
            for w in waves:
                ...
                exists=self.db.fetchone(
                    """
                    SELECT id FROM football_predictive4_price_observations
                    WHERE predictive_bet_id=? AND source_snapshot_at=?
                    """,
                    (bet["id"],wave),
                )
                ...
                self.db.execute(
                    """
                    INSERT INTO football_predictive4_price_observations(
                        predictive_bet_id,observed_at,source_snapshot_at,
                        bookmaker_key,price,move_vs_entry_pct
                    ) VALUES(?,?,?,?,?,?)
                    """,
    ...
    def finalize_clv(self, now: Optional[datetime] = None) -> int:
        ...
        for bet in bets:
            ...
            self.db.execute(
                """
                UPDATE football_predictive4_bets
                SET closing_odds=?,clv_pct=?,closing_observed_at=?,
                    closing_minutes_before_kickoff=?,clv_quality=?
                WHERE id=?
                """,
    ...
    def finalize_prediction_market_close(self, now: Optional[datetime] = None) -> int:
        ...
        for pred in preds:
            ...
            self.db.execute(
                """
                UPDATE football_predictive4_predictions
    ...
    def settle(self) -> Dict[str,int]:
        predictions=self.db.fetchall(
            ...
            FROM football_predictive4_predictions p
...
def predictive4_bootstrap_status(db: Database) -> Dict[str,Any]:
    total=int((db.fetchone("SELECT COUNT(*) AS n FROM football_pxg_current_matches WHERE home_proxy_xg IS NOT NULL AND away_proxy_xg IS NOT NULL") or {}).get("n") or 0)
    teams=int((db.fetchone("SELECT COUNT(DISTINCT team) AS n FROM (SELECT home_team AS team FROM football_pxg_current_matches WHERE home_proxy_xg IS NOT NULL UNION SELECT away_team AS team FROM football_pxg_current_matches WHERE away_proxy_xg IS NOT NULL) t") or {}).get("n") or 0)
...
def predictive4_market_funnel(db: Database) -> List[Dict[str,Any]]:
    return db.fetchall(
        """
        SELECT market_key,line_key,decision,reason,COUNT(*) AS count
        FROM football_predictive4_market_evaluations
...
def latest_predictive4_market_bets(
    ...
    return db.fetchall(
        ...
        SELECT b.*,p.league,p.home_team,p.away_team,p.commence_time,
               ...
               (SELECT o.move_vs_entry_pct
                FROM football_predictive4_market_price_observations o
...
def predictive4_funnel(db: Database) -> List[Dict[str,Any]]:
    return db.fetchall(
        """
        SELECT decision,reason,COUNT(*) AS count
        FROM football_predictive4_evaluations
...
def predictive4_league_summary(db: Database) -> List[Dict[str,Any]]:
    return db.fetchall(
        """
...
def latest_predictive4_bets(db: Database, limit: int=100) -> List[Dict[str,Any]]:
    return db.fetchall(
        """
        SELECT b.*,p.league,p.home_team,p.away_team,p.commence_time,
               p.expected_home_goals,p.expected_away_goals,
               (SELECT o.move_vs_entry_pct
                FROM football_predictive4_price_observations o
                WHERE o.predictive_bet_id=b.id
                ORDER BY o.source_snapshot_at DESC LIMIT 1) AS latest_move_pct
        FROM football_predictive4_bets b
        JOIN football_predictive4_predictions p ON p.id=b.prediction_id
